"""
🎯 Simple Forest Gump Demo with Dummy Observations

This is a minimal demonstration script that feeds dummy observations to a trained
ONNX policy to test physics timing and basic functionality without CSV playback.

Key Features:
- Uses dummy observations instead of CSV playback data
- Focuses on physics timestep investigation
- Minimal simulation setup for debugging
- Real-time ONNX inference testing

🔬 Physics Investigation:
Current issues suggest physics timestep mismatch is causing instabilities.
We'll test different physics timesteps to find optimal settings.
"""
...
log_file = f"/home/goat/Documents/GitHub/Leatherback/logs/forest_gump/simple_demo_{timestamp}.log"
...
print(f"🎯 Starting Simple Forest Gump Demo...")
print(f"📝 Debug log: {log_file}")

# 🧪 PHYSICS TIMESTEP EXPERIMENT - Try different timesteps
PHYSICS_TIMESTEPS_TO_TEST = [
    1/60,    # Current setting (16.67ms)
    1/120,   # Higher frequency (8.33ms) 
    1/240,   # Very high frequency (4.17ms)
    1/30,    # Lower frequency (33.33ms)
    0.01,    # 10ms (100Hz) - common in robotics
    0.008,   # 8ms (125Hz) - Isaac Lab default in some examples
    0.0167,  # 16.7ms (60Hz) - explicit value
]

# For this demo, we'll use the current setting but log the investigation
PHYSICS_DT = 1/60  # Will be made configurable
RENDERING_DT = 1/50

logger.info(f"🔬 PHYSICS INVESTIGATION:")
logger.info(f"   Current physics_dt: {PHYSICS_DT:.6f}s ({1/PHYSICS_DT:.1f}Hz)")
logger.info(f"   Current rendering_dt: {RENDERING_DT:.6f}s ({1/RENDERING_DT:.1f}Hz)")
logger.info(f"   Available test timesteps: {[f'{dt:.6f}s ({1/dt:.1f}Hz)' for dt in PHYSICS_TIMESTEPS_TO_TEST]}")
...
step_counter = 0
start_time = None
max_steps = 600  # Run for 10 seconds at 60Hz
dummy_target_position = [5.0, 0.0]  # Simple target 5m ahead

# Previous actions for observation history
...
def create_dummy_observations():
    ...
    Observation structure (based on training):
    obs_0: position_error (distance to target)
    obs_1: cos(heading_error) 
    obs_2: sin(heading_error)
    obs_3: linear_vel_x (body frame)
    obs_4: linear_vel_y (body frame) 
    obs_5: angular_vel_z (world frame)
    obs_6: throttle_state (previous action)
    obs_7: steering_state (previous action)
    """
    
    # Get robot state
    robot_pos, robot_rot = robot_art.get_world_poses()
    robot_pos = robot_pos[0]  # Extract position array
    robot_rot = robot_rot[0]  # Extract quaternion array
    ...
    target_heading = torch.atan2(torch.tensor(dummy_target_position[1] - robot_pos[1], dtype=torch.float32), 
                                torch.tensor(dummy_target_position[0] - robot_pos[0], dtype=torch.float32))
    ...
    obs = torch.tensor([
        ...
        previous_throttle_action,                # obs_6: throttle state
        previous_steering_action,                # obs_7: steering state
    ], dtype=torch.float32)
    
    return obs.reshape(1, -1)  # Shape: (1, 8)

def on_physics_step(step_size):
    """Physics step callback with dummy observations"""
    global step_counter, previous_throttle_action, previous_steering_action, start_time
    ...
    if start_time is None:
        ...
        logger.info(f"🚀 Physics simulation started")
    
    # Stop after max_steps
    if step_counter > max_steps:
        elapsed_time = time.time() - start_time
        logger.info(f"✅ Demo completed after {step_counter} steps in {elapsed_time:.2f}s")
        logger.info(f"📊 Average step rate: {step_counter/elapsed_time:.1f} Hz")
        logger.info(f"📊 Expected step rate: {1/PHYSICS_DT:.1f} Hz")
    ...
    previous_throttle_action = throttle_action[0]
    previous_steering_action = steering_action[0]
    
    # 🧪 PHYSICS TIMING ANALYSIS - Log timing data
    if step_counter <= 10 or step_counter % 60 == 0:  # First 10 steps + every second
        current_time = time.time()
        elapsed = current_time - start_time
        expected_time = step_counter * PHYSICS_DT
        time_drift = elapsed - expected_time
        
        logger.info(f"🕒 PHYSICS TIMING (Step {step_counter}):")
        logger.info(f"   Elapsed real time: {elapsed:.6f}s")
        logger.info(f"   Expected sim time: {expected_time:.6f}s") 
        logger.info(f"   Time drift: {time_drift:.6f}s ({time_drift/expected_time*100:.2f}%)")
        logger.info(f"   Actual step rate: {step_counter/elapsed:.1f} Hz")
        logger.info(f"   Target step rate: {1/PHYSICS_DT:.1f} Hz")
        
        # Log observations and actions for debugging
        logger.info(f"📊 State: pos_err={obs[0,0]:.3f}, cos={obs[0,1]:.3f}, sin={obs[0,2]:.3f}")
        logger.info(f"🎮 Actions: throttle_raw={throttle_raw:.6f}, steering_raw={steering_raw:.6f}")
        logger.info(f"⚙️ Scaled: throttle={throttle_action[0]:.3f}, steering={steering_action[0]:.3f}")
        
        # Check for timing issues
        if abs(time_drift) > 0.1:  # 100ms drift threshold
            logger.warning(f"⚠️ SIGNIFICANT TIMING DRIFT: {time_drift:.3f}s")
        
        # Check for extreme actions (instability indicators)
        if abs(throttle_raw) > 3.0:
            logger.warning(f"⚠️ HIGH THROTTLE MAGNITUDE: {abs(throttle_raw):.3f}")
        if abs(steering_raw) > 3.0:
            logger.warning(f"⚠️ HIGH STEERING MAGNITUDE: {abs(steering_raw):.3f}")

# Setup world with different physics timestep for testing
logger.info(f"🌍 Creating world with physics_dt={PHYSICS_DT:.6f}s, rendering_dt={RENDERING_DT:.6f}s")
...
# 🧪 FINAL PHYSICS INVESTIGATION SUMMARY
logger.info("🔬 PHYSICS INVESTIGATION SUMMARY:")
logger.info(f"   Testing physics_dt: {PHYSICS_DT:.6f}s ({1/PHYSICS_DT:.1f}Hz)")
logger.info(f"   Expected instability causes:")
logger.info(f"     - Physics timestep too large -> unstable integration")
logger.info(f"     - Physics timestep too small -> too much computation")  
logger.info(f"     - Mismatch between training and inference timesteps")
logger.info(f"   This demo will help identify optimal physics settings")

# Start simulation
logger.info("🚀 Starting simple demonstration...")
